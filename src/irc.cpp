...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #ukcoincoinTEST\r");
            Send(hSocket, "WHO #ukcoincoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #ukcoincoin\n");
            // randomly join #ukcoincoin00-#ukcoincoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #ukcoincoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ukcoincoin%02d\r", channel_number).c_str());
        };
